  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      if set_type == "test":
        ...
        gold_score = float(tokenization.convert_to_unicode(line[9]))
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, gold_score=gold_score))
    return examples
...
def convert_single_example(ex_index, example, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf.logging.info("score: %f" % (example.gold_score))
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "gold_score": tf.FixedLenFeature([], tf.float32),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 gold_scores, use_one_hot_embeddings):
  ...
  with tf.variable_scope("loss"):
    ...
    per_example_loss = tf.losses.mean_squared_error(gold_scores,scores,reduction=tf.losses.Reduction.None)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    gold_scores = features["gold_score"]
    ...
    if init_checkpoint:
      ...
      ) = BlockBert.get_assignment_map_from_checkpoint(tvars, init_checkpoint,bert_config.first_pretraining)
      if use_tpu:
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, gold_scores, scores, is_real_example):
        p_corr = tf.contrib.metrics.streaming_pearson_correlation(
            labels=gold_scores, predictions=scores, weights=is_real_example)
        loss = tf.metrics.mean(values=per_example_loss, weights=is_real_example)
        return {
            "eval_pearson_correlation_coefficient": p_corr,
            "eval_loss": loss,
        }

      eval_metrics = (metric_fn,
                      [per_example_loss, gold_scores, scores, is_real_example])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"similarity scores": scores},
          scaffold_fn=scaffold_fn)
    return output_spec
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
  ...
  for feature in features:
    ...
    all_gold_scores.append(feature.gold_score)
    ...
    d = tf.data.Dataset.from_tensor_slices({
        ...
        "segment_ids":
            tf.constant(
                ...
                dtype=tf.int32),
        "label_ids":
            tf.constant(all_gold_scores, shape=[num_examples], dtype=tf.float32),
    })
...
def main(_):
  ...
  tf.logging.info("TASK_NAME: STS-B" )
